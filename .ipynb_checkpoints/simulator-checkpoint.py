...
class CryptoExecutor:
    def __init__(self, AsmCodeInit=None, DMemInit=None, IMemInit=None, log=None, verbose=False):
        ...
        # Instruction Set of Flash Look_up_Table
        self._InstrSet.SetExec(Instr = "flkt8m8",   Func = self._Execflkt8m8  )      
        self._InstrSet.SetExec(Instr = "flkt4m4",   Func = self._Execflkt4m4  )      
        self._InstrSet.SetExec(Instr = "flkt6m4",   Func = self._Execflkt6m4  )      
        self._InstrSet.SetExec(Instr = "flkt8m4",   Func = self._Execflkt8m4  )      
        self._InstrSet.SetExec(Instr = "flkt8m32",   Func = self._Execflkt8m32  )      
        ...
        self._InstrSet.SetExec(Instr="sd32",        Func=self._Execsd32)
        self._InstrSet.SetExec(Instr="sd64",        Func=self._Execsd64)
        self._InstrSet.SetExec(Instr="sd128",        Func=self._Execsd128)
        self._InstrSet.SetExec(Instr="sd256",        Func=self._Execsd256)
        self._InstrSet.SetExec(Instr="ld32",        Func=self._Execld32)
        self._InstrSet.SetExec(Instr="ld64",        Func=self._Execld64)
        self._InstrSet.SetExec(Instr="ld128",        Func=self._Execld128)
        self._InstrSet.SetExec(Instr="ld256",        Func=self._Execld256)
 ...
 #       # Stack
    ...
    def RunAsm(self, PC=None):
        ...
        while self._RF["PC"] != None and (PC != self._AsmCode.LabelToPC('nop')):
            self._RF["PC"] = PC   # Set current PC
    ...
    def _Execflkt8m8(self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(1)
        ...
        for i in range(64):
            data = self._DataMem[Addr + i]
            s1, s2, s3, s4 = bit32_to_8(data)
            self._RF['LUT8X8_' + str(i*4 + 0)] = s4
            self._RF['LUT8X8_' + str(i*4 + 1)] = s3
            self._RF['LUT8X8_' + str(i*4 + 2)] = s2
            self._RF['LUT8X8_' + str(i*4 + 3)] = s1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execflkt4m4(self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(1)
        MemSplit = (re.match(r'(-?\w+)(\((\w+)\))?', Mem))
        ...
        for i in range(2):
            data = self._DataMem[Addr + i]
            s1, s2, s3, s4 = bit32_to_8(data)
            self._RF['LUT4X4_' + str(i*8 + 0)] = s4 & 0xf
            self._RF['LUT4X4_' + str(i*8 + 1)] = (s4>>4) & 0xf
            self._RF['LUT4X4_' + str(i*8 + 2)] = s3 & 0xf
            self._RF['LUT4X4_' + str(i*8 + 3)] = (s3>>4) & 0xf 
            self._RF['LUT4X4_' + str(i*8 + 4)] = s2 & 0xf
            self._RF['LUT4X4_' + str(i*8 + 5)] = (s2>>4) & 0xf
            self._RF['LUT4X4_' + str(i*8 + 6)] = s1 & 0xf
            self._RF['LUT4X4_' + str(i*8 + 7)] = (s1>>4) & 0xf
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execflkt6m4(self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(1)
        MemSplit = (re.match(r'(-?\w+)(\((\w+)\))?', Mem))
        ...
        for i in range(8):
            data = self._DataMem[Addr + i]
            s1, s2, s3, s4 = bit32_to_8(data)
            self._RF['LUT6X4_' + str(i*8 + 0)] = s4 & 0xf
            self._RF['LUT6X4_' + str(i*8 + 1)] = (s4>>4) & 0xf
            self._RF['LUT6X4_' + str(i*8 + 2)] = s3 & 0xf
            self._RF['LUT6X4_' + str(i*8 + 3)] = (s3>>4) & 0xf 
            self._RF['LUT6X4_' + str(i*8 + 4)] = s2 & 0xf
            self._RF['LUT6X4_' + str(i*8 + 5)] = (s2>>4) & 0xf
            self._RF['LUT6X4_' + str(i*8 + 6)] = s1 & 0xf
            self._RF['LUT6X4_' + str(i*8 + 7)] = (s1>>4) & 0xf
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execflkt8m4(self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(1)
        MemSplit = (re.match(r'(-?\w+)(\((\w+)\))?', Mem))
        ...
        for i in range(32):
            data = self._DataMem[Addr + i]
            s1, s2, s3, s4 = bit32_to_8(data)
            self._RF['LUT8X4_' + str(i*8 + 0)] = s4 & 0xf
            self._RF['LUT8X4_' + str(i*8 + 1)] = (s4>>4) & 0xf
            self._RF['LUT8X4_' + str(i*8 + 2)] = s3 & 0xf
            self._RF['LUT8X4_' + str(i*8 + 3)] = (s3>>4) & 0xf 
            self._RF['LUT8X4_' + str(i*8 + 4)] = s2 & 0xf
            self._RF['LUT8X4_' + str(i*8 + 5)] = (s2>>4) & 0xf
            self._RF['LUT8X4_' + str(i*8 + 6)] = s1 & 0xf
            self._RF['LUT8X4_' + str(i*8 + 7)] = (s1>>4) & 0xf
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execflkt8m32(self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(1)
        MemSplit = (re.match(r'(-?\w+)(\((\w+)\))?', Mem))
    ...
    def _Execld64(self, Asm="", PC=None):
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._RF[Reg] = (self._DataMem[Addr + 1]<<32) + self._DataMem[Addr]
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execld128(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._RF[Reg] = (self._DataMem[Addr+3]<<96) + (self._DataMem[Addr+2]<<64) + (self._DataMem[Addr+1]<<32) + self._DataMem[Addr]
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execld256(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._RF[Reg] = ((self._DataMem[Addr+7]<<224)+ (self._DataMem[Addr+6]<<192)+ (self._DataMem[Addr+5]<<160)+ (self._DataMem[Addr+4]<<128) +
                         (self._DataMem[Addr+3]<<96) + (self._DataMem[Addr+2]<<64) + (self._DataMem[Addr+1]<<32) + (self._DataMem[Addr]))
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execsd32(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
    ...
    def _Execsd64(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._DataMem[Addr+1] = (self._RF[Reg]>>32) & 0xffffffff
        self._DataMem[Addr] = self._RF[Reg] & 0xffffffff
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execsd128(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._DataMem[Addr+3] = (self._RF[Reg]>>96) & 0xffffffff
        self._DataMem[Addr+2] = (self._RF[Reg]>>64) & 0xffffffff
        self._DataMem[Addr+1] = (self._RF[Reg]>>32) & 0xffffffff
        self._DataMem[Addr] = self._RF[Reg] & 0xffffffff
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
    ...
    def _Execsd256(self, Asm="", PC=None):
        # Instruction behaviour
        Match = self._InstrSet.Match(Asm)
        Mem = Match.group(2)
        Reg = Match.group(1)
        ...
        Addr = self._RF[Pt] + Offset
        self._Log.write("LOAD  From Memory Address: %x From Reg %s, Value is %x \n" % (Addr, Reg, self._DataMem[Addr]))
        self._DataMem[Addr+7] = (self._RF[Reg]>>224) & 0xffffffff
        self._DataMem[Addr+6] = (self._RF[Reg]>>192) & 0xffffffff
        self._DataMem[Addr+5] = (self._RF[Reg]>>160) & 0xffffffff
        self._DataMem[Addr+4] = (self._RF[Reg]>>128) & 0xffffffff
        self._DataMem[Addr+3] = (self._RF[Reg]>>96) & 0xffffffff
        self._DataMem[Addr+2] = (self._RF[Reg]>>64) & 0xffffffff
        self._DataMem[Addr+1] = (self._RF[Reg]>>32) & 0xffffffff
        self._DataMem[Addr] = self._RF[Reg] & 0xffffffff
    ...
    def _Execqand    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffff
            s2 = self._RF[r2] & 0xffffffff
    ...
    def _Execqand64    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffff
            s2 = self._RF[r2] & 0xffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, s1, r2, s2))
            self._RF[ret1] = result = s1 & s2
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqand128    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            s2 = self._RF[r2] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, s1, r2, s2))
            self._RF[ret] = result = s1 & s2
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqand_mask    (self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        ret, r1, r2, mask = Match.group(1), Match.group(2), Match.group(3), Match.group(4)
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2) or self._RF.IsLocked(mask):
            return False, PC
        else:
            s1 = self._RF[r1]
            s2 = self._RF[r2]
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, s1, r2, s2))
            tempt = s1 & s2
            result = self._RF[ret] = mask_op(tempt, mask)
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqnot    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) :
            ...
            s1 = self._RF[r1] & 0xffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, s1))
            result = self._RF[ret] = 0xffffffff - s1
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqnot64    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) :
            ...
            s1 = self._RF[r1] & 0xffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, s1))
            self._RF[ret] = result = 0xffffffffffffffff - s1
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqnot128    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) :
            ...
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, s1))
            result = 0xffffffffffffffffffffffffffffffff - s1
            self._RF[ret] = 0xffffffffffffffffffffffffffffffff - s1
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqtli    (self, Asm="", PC=None):
        ...
        if re.match('-?0x', Match.group(3)):
            ...
            imm = int(Match.group(3))
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or (imm>255):
            return False, PC
        else:
            log = bit8(imm)
            A = self._RF[r1] & 0xffffffff
            B = (self._RF[r1]>>32) & 0xffffffff
            C = (self._RF[r1]>>64) & 0xffffffff
            self._Log.write("Source Reg 1 : %s = %x \nSource Reg 2 : %s = %x  \n"%(r1, self._RF[r1], r2, self._RF[r2]))
            A_inv = 0xffffffff - A
            B_inv = 0xffffffff - B
            C_inv = 0xffffffff - C
            result = self._RF[ret] = ( (log[7] * (A & B & C)) | (log[6] * (A & B & C_inv)) |
    ...
    def _Execqtl    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            log = bit32(self._RF[r2] & 0xffffffff)
            A = (self._RF[r2]>>32) & 0xffffffff
            B = self._RF[r1] & 0xffffffff
            C = (self._RF[r1]>>32) & 0xffffffff
            A_inv = 0xffffffff - A
            B_inv = 0xffffffff - B
            C_inv = 0xffffffff - C
            self._Log.write("Source Reg 1 : %s = %x \nSource Reg 2 : %s = %x  \n"%(r1, self._RF[r1], r2, self._RF[r2]))
    ...
    def _Execqsxor    (self, Asm="", PC=None):
        ...
        rd = split.group(1)
        ret, r1, r2 = rd, Match.group(2), Match.group(3)
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffff
            ...
            for i in range(32) :
                if log2[i]:
                    tempt = tempt ^ log1[i]
            result = self._RF[ret1] = tempt * pow(2, j)
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqcmp    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, s1, r2, s2))
            if s1 < s2:
                result = self._RF[ret] = 1
            else :
                result = self._RF[ret] = 0
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqtesti    (self, Asm="", PC=None):
        ...
        if re.match('-?0x', Match.group(3)):
            ...
            imm = int(Match.group(3))
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or imm>31:
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n Imm: %x  \n"%(r1, s1, imm))
    ...
    def _Execqtesti64    (self, Asm="", PC=None):
        ...
        if re.match('-?0x', Match.group(3)):
            ...
            imm = int(Match.group(3))
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or imm>63:
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nImm : %x  \n"%(r1, s1, imm))
    ...
    def _Execqtesti128    (self, Asm="", PC=None):
        ...
        if re.match('-?0x', Match.group(3)):
            ...
            imm = int(Match.group(3))
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or imm>127:
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nImm : %x  \n"%(r1, s1, imm))
    ...
    def _Execqteqi    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            s1 = self._RF[r1]
            self._Log.write("Source Reg 1 : %s = %x  \nImm : %x  \n"%(r1, s1, imm))
            if ((s1==0) & (imm==0)) or (s1==0xffffffff & (imm==1)):
                result = self._RF[ret] = 1
            else :
                result = self._RF[ret] = 0
            # Information to be reported
    ...
    def _Execqteqi64    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nImm : %x  \n"%(r1, s1, imm))
            if ((s1==0) & (imm==0)) or (s1==0xffffffffffffffff & (imm==1)):
                result = self._RF[ret] = 1
            else :
                result = self._RF[ret] = 0
            # Information to be reported
    ...
    def _Execqteqi128    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nImm : %x  \n"%(r1, s1, imm))
            if ((s1==0) & (imm==0)) or (s1==0xffffffffffffffffffffffffffffffff & (imm==1)):
                result = self._RF[ret] = 1
            else :
                result = self._RF[ret] = 0
            # Information to be reported
    ...
    def _Execqteq    (self, Asm="", PC=None):
        # Instruction Behaviour
        Match = self._InstrSet.Match(Asm)
        ret, r1, r2, r3 = Match.group(1), Match.group(2), Match.group(3), Match.group(4)
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2) or self._RF.IsLocked(r3):
            return False, PC
        else:
            s1 = self._RF[r1] & 0xffffffff
            s2 = self._RF[r2] & 0xffffffff
            s3 = self._RF[r3] & 0xffffffff
            self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, s1, r2, s2, r3, s3))
            if ( (s1&(1<<s3)) == (s2&(1<<s3)) ):
                result = self._RF[ret] = 1
            else :
    ...
    def _Execqteq64    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2) or self._RF.IsLocked(r3):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffff
            s2 = self._RF[r2] & 0xffffffffffffffff
            s3 = self._RF[r3] & 0xffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \nReg 3 : %s = %x  \n"%(r1, s1, r2, s2, r3, s3))
            if ( (s1&(1<<s3)) == (s2&(1<<s3)) ):
                result = self._RF[ret] = 1
            else :
    ...
    def _Execqteq128    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2) or self._RF.IsLocked(r3):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            s2 = self._RF[r2] & 0xffffffffffffffffffffffffffffffff
            s3 = self._RF[r3] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \nReg 3 : %s = %x  \n"%(r1, s1, r2, s2, r3, s3))
            if ((s1&(1<<s3)) == (s2&(1<<s3)) ):
                result = self._RF[ret] = 1
            else :
    ...
    def _Execqmodadd8(self, Asm="", PC=None):
        ...
        result = self._RF[ret] = pow(s1+s2, 1, (1<<self._RF[r3])+self._RF[r4])
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmodadd16(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1, r2, r3, r4 = Match.group(1), Match.group(2), Match.group(3), Match.group(4), Match.group(5)
        s1 = self._RF[r1] & 0xffff
        s2 = self._RF[r2] & 0xffff
        self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \nReg 2 : %s = %x  \nReg 3 : %s = %x  \n"%(r1, s1, r2, s2, r3, self._RF[r3], r4, self._RF[r4]))		
        result = self._RF[ret] = pow(s2+s4, 1, (1<<self._RF[r3])+self._RF[r4])
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmodsub8(self, Asm="", PC=None):
        ...
        s2 = self._RF[r2] & 0xff
        self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \nReg 2 : %s = %x  \nReg 3 : %s = %x  \n"%(r1, s1, r2, s2, r3, self._RF[r3], r4, self._RF[r4]))
        result = self._RF[ret] = pow(s4-s8, 1, (1<<self._RF[r3])+self._RF[r4])
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmodsub128(self, Asm="", PC=None):
        ...
        s2 = self._RF[r2] & 0xffffffffffffffffffffffffffffffff
        self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \nReg 2 : %s = %x  \nReg 3 : %s = %x  \n"%(r1, s1, r2, s2, r3, self._RF[r3], r4, self._RF[r4]))
        self._RF[ret] = result = pow(s1-s2, 1, (1<<self._RF[r3]) + self._RF[r4])
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmul8l(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1, r2 = Match.group(1), Match.group(2), Match.group(3)
        s1, s2, s3, s4 = bit32_to_8(self._RF[r1] & 0xffffffff)
        s5, s6, s7, s8 = bit32_to_8(self._RF[r2] & 0xffffffff)
        self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \n" % (r1, (s3<<8)+s4, r2, (s7<<8)+s8))
        result = self._RF[ret] = (s3 * s7) * pow(2, 16) + (s4 * s8)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmul8h(self, Asm="", PC=None):
        ...
        s1, s2, s3, s4 = bit32_to_8(self._RF[r1] & 0xffffffff)
        s5, s6, s7, s8 = bit32_to_8(self._RF[r2] & 0xffffffff)
        self._Log.write("Source Reg 0 : %s = %x  \nSource Reg 1 : %s = %x  \n" % (r1, (s1<<8)+s2, r2, (s5<<8)+s6))
        result = self._RF[ret] = (s1 * s5) * pow(2, 16) + (s2 * s6)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmul16l(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1, r2 = Match.group(1), Match.group(2), Match.group(3)
        s1, s2 = bit32_to_16(self._RF[r1] & 0xffffffff)
        s3, s4 = bit32_to_16(self._RF[r2] & 0xffffffff)
        self._Log.write("Source Reg 1 : %s = %x  \nReg 2: %s = %x  \n" % (r1, s2, r2, s4))
        result = self._RF[ret] = s2 * s4
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgfinv(self, Asm="", PC=None):
        ...
        result = self._RF[ret] = findModReverse(tempt, self._RF['inv'])
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmodmul8(self, Asm="", PC=None):
        ...
        result = self._RF[ret] = pow(tempt, 1, n)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqishl32(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 1 : %s = %x  \nImm: %x  \n" % (r1, s1, imm))
        result = self._RF[ret] = int(((((bin(s1<<imm)[2:])[::-1])[0:32])[::-1]), 2)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqishl8(self, Asm="", PC=None):
        ...
        if re.match('-?0x', Match.group(3)):
            ...
            imm = int(Match.group(3))
        s1 = self._RF[r1] & 0xff
        s2 = (self._RF[r1]>>8) & 0xff
        s3 = (self._RF[r1]>>16) & 0xff
        s4 = (self._RF[r1]>>24) & 0xff
        self._Log.write("Source Reg 1 : %s = %x  \nimm : %x  \n" % (r1, self._RF[r1], imm))
        r1_L_16_L_8 = (s1<<imm)[2:][::-1][0:8][::-1]
        r1_L_16_H_8 = (s2<<imm)[2:][::-1][0:8][::-1]
        r1_H_16_L_8 = (s3<<imm)[2:][::-1][0:8][::-1]
        r1_H_16_H_8 = (s4<<imm)[2:][::-1][0:8][::-1]
        result = self._RF[ret] = (r1_H_16_H_8<<24) + (r1_H_16_L_8<<16) + (r1_L_16_H_8<<8) + (r1_L_16_L_8)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqshl32(self, Asm="", PC=None):
        ...
        result = self._RF[ret] = int(bin(s1<<shift)[2:][::-1][0:32][::-1], 2)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqshl8(self, Asm="", PC=None):
        ...
        s4 = (self._RF[r1]>>24) & 0xff
        self._Log.write("Source Reg 1 : %s = %x  \nReg 2 : %s = %x  \n" % (r1, self._RF[r1], r2, self._RF[r2]))
        r1_L_16_L_8 = (s1<<shift)[2:][::-1][0:8][::-1]
        r1_L_16_H_8 = (s2<<shift)[2:][::-1][0:8][::-1]
        r1_H_16_L_8 = (s3<<shift)[2:][::-1][0:8][::-1]
        r1_H_16_H_8 = (s4<<shift)[2:][::-1][0:8][::-1]
        result = self._RF[ret + '_e0'] = (r1_H_16_H_8<<24) + (r1_H_16_L_8<<16) + (r1_L_16_H_8<<8) + (r1_L_16_L_8)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqshr8(self, Asm="", PC=None):
        ...
        s3 = (self._RF[r1]>>16) & 0xff
        s4 = (self._RF[r1]>>24) & 0xff
        r1_L_16_L_8 = s1>>shift
        r1_L_16_H_8 = s2>>shift
        r1_H_16_L_8 = s3>>shift
        r1_H_16_H_8 = s4>>shift
        result = self._RF[ret] = (r1_H_16_H_8<<24) + (r1_H_16_L_8<<16) + (r1_L_16_H_8<<8) + (r1_L_16_L_8)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
    ...
    def _Execqirol32(self, Asm="", PC=None):
        ...
        if imm == 0:
            ...
            result = self._RF[ret] = (shift2<<imm) + shift1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqrol32(self, Asm="", PC=None):
        ...
        if (shift == 0):
            ...
            result = self._RF[ret] = (shift2<<shift) + shift1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqiror32(self, Asm="", PC=None):
        ...
        if imm == 0:
            ...
            result = self._RF[ret] = (shift2<<(32-imm)) + shift1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqror32(self, Asm="", PC=None):
        ...
        if (shift == 0):
            ...
            result = self._RF[ret] = (shift2<<(32 - shift)) + shift1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqsbitperm32(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r0= Match.group(1), Match.group(2)
        s0 = self._RF[r0] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = (self._RF[r0 + '_e0'] >> (self._RF["perm"] & 0x1f) & 0b1) | (
    ...
    def _Execqsbitperm64(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r0= Match.group(1), Match.group(2)
        s0 = self._RF[r0] & 0xffffffffffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
    ...
    def _Execqsbitperm128(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r0= Match.group(1), Match.group(2)
        s0 = self._RF[r0] & 0xffffffffffffffffffffffffffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = (s0 >> (self._RF["perm"] & 0x1f) & 0b1) | (
    ...
    def _Execqbyteperm64(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        self._RF[ret] = bit64_to_32((s0 >> ((s1 & 0b111) * 8) & 0xff) | (
                (s0 >> ((s1 >> 3 & 0b111) * 8) & 0xff) << 8) | (
                                (s0 >> ((s1 >> 6 & 0b111) * 8) & 0xff) << 16) | (
                                (s0 >> ((s1 >> 9 & 0b111) * 8) & 0xff) << 24) | (
                (s0 >> ((s1 >> 12 & 0b111) * 8) & 0xff) << 32) | (
                                (s0 >> ((s1 >> 15 & 0b111) * 8) & 0xff) << 40) | (
                                (s0 >> ((s1 >> 18 & 0b111) * 8) & 0xff) << 48) | (
                (s0 >> ((s1 >> 21 & 0b111) * 8) & 0xff) << 56))
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqbyteperm128(self, Asm="", PC=None):
        ...
        self._Log.write(
            "Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        self._RF[ret] = (s0 >> ((s1 & 0xf) * 8) & 0xff) | (
                (s0 >> ((s1 >> 4 * 1 & 0xf) * 8) & 0xff) << 8 * 1) | (
                                (s0 >> ((s1 >> 4 * 2 & 0xf) * 8) & 0xff) << 8 * 2) | (
                                (s0 >> ((s1 >> 4 * 3 & 0xf) * 8) & 0xff) << 8 * 3) | (
                                (s0 >> ((s1 >> 4 * 4 & 0xf) * 8) & 0xff) << 8 * 4) | (
                                (s0 >> ((s1 >> 4 * 5 & 0xf) * 8) & 0xff) << 8 * 5) | (
                                (s0 >> ((s1 >> 4 * 6 & 0xf) * 8) & 0xff) << 8 * 6) | (
                                (s0 >> ((s1 >> 4 * 7 & 0xf) * 8) & 0xff) << 8 * 7) | (
                                (s0 >> ((s1 >> 4 * 8 & 0xf) * 8) & 0xff) << 8 * 8) | (
                                (s0 >> ((s1 >> 4 * 9 & 0xf) * 8) & 0xff) << 8 * 9) | (
                                (s0 >> ((s1 >> 4 * 10 & 0xf) * 8) & 0xff) << 8 * 10) | (
                                (s0 >> ((s1 >> 4 * 11 & 0xf) * 8) & 0xff) << 8 * 11) | (
                                (s0 >> ((s1 >> 4 * 12 & 0xf) * 8) & 0xff) << 8 * 12) | (
                                (s0 >> ((s1 >> 4 * 13 & 0xf) * 8) & 0xff) << 8 * 13) | (
                                (s0 >> ((s1 >> 4 * 14 & 0xf) * 8) & 0xff) << 8 * 14) | (
                                (s0 >> ((s1 >> 4 * 15 & 0xf) * 8) & 0xff) << 8 * 15)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqbitsw(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        temp1 = (s0 >> (s1 & 0x1f) & 1) << (s1 >> 5 & 0x1f)
        temp2 = (s0 >> (s1 >> 5 & 0x1f) & 1) << (s1 & 0x1f)
        self._RF[ret] = s0 & ~(1 <<(s1 & 0x1f)) & ~(1 << (s1 >> 5 & 0x1f)) | (temp1 | temp2)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqbytesw(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1]
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        temp1 = (s0 >> ((s1 & 0b11) * 8) & 0xff) << ((s1 >> 2 & 0b11) * 8)
        temp2 = (s0 >> ((s1 >> 2 & 0b11) * 8) & 0xff) << ((s1 & 0b11) * 8)
        self._RF[ret] = s0 & ~(0xff << ((s1 & 0b11) * 8)) & ~(0xff << ((s1 >> 2 & 0b11) * 8)) | (temp1 | temp2)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlut8m8(self, Asm="", PC=None):
        ...
        ret, r0= Match.group(1), Match.group(2)
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, self._RF[r0]))
        s1, s2, s3, s4 = bit32_to_8(self._RF[r0] & 0xffffffff)
    ...
    def _Execq4lut8m8(self, Asm="", PC=None):
        Match = self._InstrSet.Match(Asm)
        ret, r0= Match.group(1), Match.group(2)
        s11, s12, s13, s14 = bit32_to_8((self._RF[r0] >> 96) & 0xffffffff)
        s21, s22, s23, s24 = bit32_to_8((self._RF[r0] >> 64) & 0xffffffff)
        s31, s32, s33, s34 = bit32_to_8((self._RF[r0] >> 32) & 0xffffffff)
        s41, s42, s43, s44 = bit32_to_8(self._RF[r0] & 0xffffffff)
        ...
        rd1 = (s11_s<<24) + (s12_s<<16) + (s13_s<<8) + (s14_s)
        rd2 = (s21_s<<24) + (s22_s<<16) + (s23_s<<8) + (s24_s)
        rd3 = (s31_s<<24) + (s32_s<<16) + (s33_s<<8) + (s34_s)
        rd4 = (s41_s<<24) + (s42_s<<16) + (s43_s<<8) + (s44_s)
        self._RF[ret] = tempt = (rd1<<96) + (rd2<<64) + (rd3<<32) + (rd4)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlut4m4(self, Asm="", PC=None):
        ...
        s0 = self._RF[r0] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = self._RF["LUT4X4_"+str(s0 & 0xf)] | (self._RF["LUT4X4_"+str(s0 >> 4 & 0xf)] << 4) |\
                        (self._RF["LUT4X4_" + str(s0 >> 8 & 0xf)] << 8) | (self._RF["LUT4X4_"+str(s0 >> 12 & 0xf)] << 12) | \
                        (self._RF["LUT4X4_" + str(s0 >> 16 & 0xf)] << 16) | (self._RF["LUT4X4_" + str(s0 >> 20 & 0xf)] << 20) | \
                        (self._RF["LUT4X4_" + str(s0 >> 24 & 0xf)] << 24) | (self._RF["LUT4X4_" + str(s0 >> 28 & 0xf)] << 28)
            # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlut6m4(self, Asm="", PC=None):
        ...
        s0 = self._RF[r0] & 0xffffffffffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = self._RF["LUT6X4_"+str(s0 & 0x3f)] | (self._RF["LUT6X4_"+str(s0 >> 6 & 0x3f)] << 4) |\
                        (self._RF["LUT6X4_" + str(s0 >> 12 & 0x3f)] << 8) | (self._RF["LUT6X4_"+str(s0 >> 18 & 0x3f)] << 12) | \
                        (self._RF["LUT6X4_" + str(s0 >> 24 & 0x3f)] << 16) | (self._RF["LUT6X4_" + str(s0 >> 30 & 0x3f)] << 20) | \
                        (self._RF["LUT6X4_" + str(s0 >> 36 & 0x3f)] << 24) | (self._RF["LUT6X4_" + str(s0 >> 42 & 0x3f)] << 28)
            # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlut8m4(self, Asm="", PC=None):
        ...
        s0 = self._RF[r0] & 0xffffffffffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = self._RF["LUT8X4_"+str(s0 & 0xff)] | (self._RF["LUT8X4_"+str(s0 >> 8 & 0xff)] << 4) |\
                        (self._RF["LUT8X4_" + str(s0 >> 16 & 0xff)] << 8) | (self._RF["LUT8X4_"+str(s0 >> 24 & 0xff)] << 12) | \
                        (self._RF["LUT8X4_" + str(s0 >> 32 & 0xff)] << 16) | (self._RF["LUT8X4_" + str(s0 >> 40 & 0xff)] << 20) | \
                        (self._RF["LUT8X4_" + str(s0 >> 48 & 0xff)] << 24) | (self._RF["LUT8X4_" + str(s0 >> 56 & 0xff)] << 28)
            # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlut8m32(self, Asm="", PC=None):
        ...
        s0 = self._RF[r0] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, s0))
        self._RF[ret] = self._RF["LUT8X32_"+str(s0 >> (int(n)*8) & 0xff)]
            # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execq4lut8m32(self, Asm="", PC=None):
        ...
        s0 = self._RF[r0] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \n" % (r0, self._RF[r0 + '_e0']))
        self._RF[ret] = self._RF["LUT8X32_"+str(s0 & 0xff)] | (self._RF["LUT8X32_"+str(s0 >> 8 & 0xff)] << 32) |\
                        (self._RF["LUT8X32_" + str(s0 >> 16 & 0xff)] << 64) | (self._RF["LUT8X32_"+str(s0 >> 24 & 0xff)] << 96)
            # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqlutsw(self, Asm="", PC=None):
        ...
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        if idx == 0:
            tmp = self._RF["LUT8X8_" + str(s0 & 0xff)]
            self._RF["LUT8X8_" + str(s0 & 0xff)] = self._RF["LUT8X8_" + str(s1 & 0xff)]
            self._RF["LUT8X8_" + str(s1 & 0xff)] = tmp
        elif idx == 1:
            tmp = self._RF["LUT4X4_" + str(s0 & 0xf)]
            self._RF["LUT4X4_" + str(s0 & 0xf)] = self._RF["LUT4X4_" + str(s1 & 0xf)]
            self._RF["LUT4X4_" + str(s1 & 0xf)] = tmp
        elif idx == 2:
            tmp = self._RF["LUT6X4_" + str(s0 & 0x3f)]
            self._RF["LUT6X4_" + str(s0 & 0x3f)] = self._RF["LUT6X4_" + str(s1 & 0x3f)]
            self._RF["LUT6X4_" + str(s1 & 0x3f)] = tmp
        elif idx == 3:
            tmp = self._RF["LUT8X4_" + str(s0 & 0xff)]
            self._RF["LUT8X4_" + str(s0 & 0xff)] = self._RF["LUT8X4_" + str(s1 & 0xff)]
            self._RF["LUT8X4_" + str(s1 & 0xff)] = tmp
        else:
            tmp = self._RF["LUT8X32_" + str(s0 & 0xff)]
            self._RF["LUT8X32_" + str(s0 & 0xff)] = self._RF["LUT8X32_" + str(s1 & 0xff)]
            self._RF["LUT8X32_" + str(s1 & 0xff)] = tmp
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_3mulx(self, Asm="", PC=None):
        ...
        if (self._RF["gen_poly2_8_3"] & 0b100) == 4:
            tmp3 = op << 16
        tmp = tmp1 | tmp2 | tmp3
        self._RF[ret] = ((s0 & 0xffff) << 8) ^ tmp
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
        self.IncreaseCycles(Instr, "Parallel", self._InstrSet.GetCycles(Instr))
        self._Log.write("Result : %s = 0x%x \n" % (ret, self._RF[ret + '_e0']))
        return True, PC + 1
    ...
    def _Execqgf8_4mulx(self, Asm="", PC=None):
        ...
        self._RF[ret] = ((s0 & 0xffffff) << 8) ^ tmp
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_2mulv(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        tmp1 = gmul8_2((s0 >> 16) & 0xffff, (s1 >> 16) & 0xffff, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_2"])
        tmp2 = gmul8_2(s0 & 0xffff, s1 & 0xffff, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_2"])
        tmp3 = gmul8_2((s0 >> 16) & 0xffff, 0x0100, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_2"])
        tmp4 = gmul8_2(s0 & 0xffff, 0x0100, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_2"])
        result = tmp2 | (tmp1 << 16) | (tmp4 << 32) | (tmp3 << 48)
    ...
    def _Execqgf8_3mulv(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        tmp1 = gmul8_3(s0 & 0xffffff, s1 & 0xffffff, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_3"])
        tmp2 = gmul8_3(s0 & 0xffffff, 0x0100, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_3"])
        tmp3 = gmul8_3(s0 & 0xffffff, 0x010000, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_3"])
        self._RF[ret] = result = tmp1 | (tmp2 << 32) | (tmp3 << 64)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_4mulv(self, Asm="", PC=None):
        ...
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 0 : %s = 0x%x \nSource Reg 1 : %s = 0x%x  \n" % (r0, s0, r1, s1))
        tmp1 = gmul8_4(s0, s1, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_4"])
        tmp2 = gmul8_4(s0, 0x0100, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_4"])
        tmp3 = gmul8_4(s0, 0x010000, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_4"])
        tmp4 = gmul8_4(s0, 0x01000000, self._RF["gen_poly2_8"] | (1<<8), self._RF["gen_poly2_8_4"])
        self._RF[ret] = result = tmp1 | (tmp2 << 32) | (tmp3 << 64) | (tmp4 << 96)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmds4(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1 = Match.group(1), Match.group(2)
        d1 = self._RF[ret] & 0xffffffff
        s1 = self._RF[r1] & 0xffffffff
        self._Log.write("Source Reg 1 : %s = %x  \n Ret 1 : %s = %x  \n" % (r1, s1, ret1, d1))
        ret_H_16_H_8, ret_H_16_L_8, ret_L_16_H_8, ret_L_16_L_8 = bit32_to_8(d1)
        mds32 = bit32(s1)
        ret_H_16_H_8 = (ret_H_16_H_8 * mds32[31]) ^ (ret_H_16_L_8 * mds32[27]) ^ (ret_L_16_H_8 * mds32[23]) ^ (ret_L_16_L_8 * mds32[19])
        ret_H_16_L_8 = (ret_H_16_H_8 * mds32[30]) ^ (ret_H_16_L_8 * mds32[26]) ^ (ret_L_16_H_8 * mds32[22]) ^ (ret_L_16_L_8 * mds32[18])
        ret_L_16_H_8 = (ret_H_16_H_8 * mds32[29]) ^ (ret_H_16_L_8 * mds32[25]) ^ (ret_L_16_H_8 * mds32[21]) ^ (ret_L_16_L_8 * mds32[17])
        ret_L_16_L_8 = (ret_H_16_H_8 * mds32[28]) ^ (ret_H_16_L_8 * mds32[24]) ^ (ret_L_16_H_8 * mds32[20]) ^ (ret_L_16_L_8 * mds32[16])
        result = self._RF[ret] = (ret_H_16_H_8<<24) + (ret_H_16_L_8<<16) + (ret_L_16_H_8<<8) + (ret_L_16_L_8)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmds8(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1 = Match.group(1), Match.group(2)
        d1 = self._RF[ret] & 0xffffffffffffffff
        s1 = self._RF[r1] & 0xffffffffffffffff 
        self._Log.write("Source Reg 1 : %s = %x  \nRet1 : %s = %x  \n" % (r1, s1, ret, d1))
        ret1_H_16_H_8, ret1_H_16_L_8, ret1_L_16_H_8, ret1_L_16_L_8 = bit32_to_8((self._RF[ret]>>32) & 0xffffffff)
        ret2_H_16_H_8, ret2_H_16_L_8, ret2_L_16_H_8, ret2_L_16_L_8 = bit32_to_8(self._RF[ret] & 0xffffffff)
        mds32_r1 = bit32((self._RF[r1]>>32) & 0xffffffff)
        mds32_r2 = bit32(self._RF[r1] & 0xffffffff)
        ret1_H_16_H_8 = (ret1_H_16_H_8 * mds32_r1[31]) ^ (ret1_H_16_L_8 * mds32_r1[23]) ^ (ret1_L_16_H_8 * mds32_r1[15]) ^ (ret1_L_16_L_8 * mds32_r1[7]) ^ (ret2_H_16_H_8 * mds32_r2[31]) ^ (ret2_H_16_L_8 * mds32_r2[23]) ^ (ret2_L_16_H_8 * mds32_r2[15]) ^ (ret2_L_16_L_8 * mds32_r2[7])
        ret1_H_16_L_8 = (ret1_H_16_H_8 * mds32_r1[30]) ^ (ret1_H_16_L_8 * mds32_r1[22]) ^ (ret1_L_16_H_8 * mds32_r1[14]) ^ (ret1_L_16_L_8 * mds32_r1[6]) ^ (ret2_H_16_H_8 * mds32_r2[30]) ^ (ret2_H_16_L_8 * mds32_r2[22]) ^ (ret2_L_16_H_8 * mds32_r2[14]) ^ (ret2_L_16_L_8 * mds32_r2[6])
        ret1_L_16_H_8 = (ret1_H_16_H_8 * mds32_r1[29]) ^ (ret1_H_16_L_8 * mds32_r1[21]) ^ (ret1_L_16_H_8 * mds32_r1[13]) ^ (ret1_L_16_L_8 * mds32_r1[5]) ^ (ret2_H_16_H_8 * mds32_r2[29]) ^ (ret2_H_16_L_8 * mds32_r2[21]) ^ (ret2_L_16_H_8 * mds32_r2[13]) ^ (ret2_L_16_L_8 * mds32_r2[5])
        ret1_L_16_L_8 = (ret1_H_16_H_8 * mds32_r1[28]) ^ (ret1_H_16_L_8 * mds32_r1[20]) ^ (ret1_L_16_H_8 * mds32_r1[12]) ^ (ret1_L_16_L_8 * mds32_r1[4]) ^ (ret2_H_16_H_8 * mds32_r2[28]) ^ (ret2_H_16_L_8 * mds32_r2[20]) ^ (ret2_L_16_H_8 * mds32_r2[12]) ^ (ret2_L_16_L_8 * mds32_r2[4])
        ret2_H_16_H_8 = (ret1_H_16_H_8 * mds32_r1[27]) ^ (ret1_H_16_L_8 * mds32_r1[19]) ^ (ret1_L_16_H_8 * mds32_r1[11]) ^ (ret1_L_16_L_8 * mds32_r1[3]) ^ (ret2_H_16_H_8 * mds32_r2[27]) ^ (ret2_H_16_L_8 * mds32_r2[19]) ^ (ret2_L_16_H_8 * mds32_r2[11]) ^ (ret2_L_16_L_8 * mds32_r2[3])
        ret2_H_16_L_8 = (ret1_H_16_H_8 * mds32_r1[26]) ^ (ret1_H_16_L_8 * mds32_r1[18]) ^ (ret1_L_16_H_8 * mds32_r1[10]) ^ (ret1_L_16_L_8 * mds32_r1[2]) ^ (ret2_H_16_H_8 * mds32_r2[26]) ^ (ret2_H_16_L_8 * mds32_r2[18]) ^ (ret2_L_16_H_8 * mds32_r2[10]) ^ (ret2_L_16_L_8 * mds32_r2[2])
        ret2_L_16_H_8 = (ret1_H_16_H_8 * mds32_r1[25]) ^ (ret1_H_16_L_8 * mds32_r1[17]) ^ (ret1_L_16_H_8 * mds32_r1[9]) ^ (ret1_L_16_L_8 * mds32_r1[1]) ^ (ret2_H_16_H_8 * mds32_r2[25]) ^ (ret2_H_16_L_8 * mds32_r2[17]) ^ (ret2_L_16_H_8 * mds32_r2[9]) ^ (ret2_L_16_L_8 * mds32_r2[1])
        ret2_L_16_L_8 = (ret1_H_16_H_8 * mds32_r1[24]) ^ (ret1_H_16_L_8 * mds32_r1[16]) ^ (ret1_L_16_H_8 * mds32_r1[8]) ^ (ret1_L_16_L_8 * mds32_r1[0]) ^ (ret2_H_16_H_8 * mds32_r2[24]) ^ (ret2_H_16_L_8 * mds32_r2[16]) ^ (ret2_L_16_H_8 * mds32_r2[8]) ^ (ret2_L_16_L_8 * mds32_r2[0])
        rd1 = (ret1_H_16_H_8<<24) + (ret1_H_16_L_8<<16) + (ret1_L_16_H_8<<8) + ret1_L_16_L_8
        rd2 = (ret2_H_16_H_8<<24) + (ret2_H_16_L_8<<16) + (ret2_L_16_H_8<<8) + ret2_L_16_L_8
        result = self._RF[ret] = (rd1<<32) + rd2
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqmds16(self, Asm="", PC=None):
        ...
        Match = self._InstrSet.Match(Asm)
        ret, r1 = Match.group(1), Match.group(2)
        ret1_H_16_H_8, ret1_H_16_L_8, ret1_L_16_H_8, ret1_L_16_L_8 = bit32_to_8((self._RF[ret]>>96) & 0xffffffff)
        ret2_H_16_H_8, ret2_H_16_L_8, ret2_L_16_H_8, ret2_L_16_L_8 = bit32_to_8((self._RF[ret]>>64) & 0xffffffff)
        ret3_H_16_H_8, ret3_H_16_L_8, ret3_L_16_H_8, ret3_L_16_L_8 = bit32_to_8((self._RF[ret]>>32) & 0xffffffff)
        ret4_H_16_H_8, ret4_H_16_L_8, ret4_L_16_H_8, ret4_L_16_L_8 = bit32_to_8(self._RF[ret] & 0xffffffff)
        rs1 = self._RF[r1]
        rd1 = self._RF[ret] & 0xffffffffffffffffffffffffffffffff
        self._Log.write("Source Reg 1 : %s = %x  \nRet 1 : %s = %x  \n" % (r1, rs1, ret, rd1))
        mds32_r1 = bit32((self._RF[r1]>>224) & 0xffffffff)
        mds32_r2 = bit32((self._RF[r1]>>192) & 0xffffffff)
        mds32_r3 = bit32((self._RF[r1]>>160) & 0xffffffff)
        mds32_r4 = bit32((self._RF[r1]>>128) & 0xffffffff)
        mds32_r5 = bit32((self._RF[r1]>>96) & 0xffffffff)
        mds32_r6 = bit32((self._RF[r1]>>64) & 0xffffffff)
        mds32_r7 = bit32((self._RF[r1]>>32) & 0xffffffff)
        mds32_r8 = bit32(self._RF[r1] & 0xffffffff)
        ret1_H_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[31]) ^ (ret1_H_16_L_8 * mds32_r1[15]) ^ (ret1_L_16_H_8 * mds32_r2[31]) ^ (ret1_L_16_L_8 * mds32_r2[15]) ^ (ret2_H_16_H_8 * mds32_r3[31]) ^ (ret2_H_16_L_8 * mds32_r3[15]) ^ (ret2_L_16_H_8 * mds32_r4[31]) ^ (ret2_L_16_L_8 * mds32_r4[15]) ^ (ret3_H_16_H_8 * mds32_r5[31]) ^ (ret3_H_16_L_8 * mds32_r5[15]) ^ (ret3_L_16_H_8 * mds32_r6[31]) ^ (ret3_L_16_L_8 * mds32_r6[15]) ^ (ret4_H_16_H_8 * mds32_r7[31]) ^ (ret4_H_16_L_8 * mds32_r7[15]) ^ (ret4_L_16_H_8 * mds32_r8[31]) ^ (ret4_L_16_L_8 * mds32_r8[15])
        ret1_H_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[30]) ^ (ret1_H_16_L_8 * mds32_r1[14]) ^ (ret1_L_16_H_8 * mds32_r2[30]) ^ (ret1_L_16_L_8 * mds32_r2[14]) ^ (ret2_H_16_H_8 * mds32_r3[30]) ^ (ret2_H_16_L_8 * mds32_r3[14]) ^ (ret2_L_16_H_8 * mds32_r4[30]) ^ (ret2_L_16_L_8 * mds32_r4[14]) ^ (ret3_H_16_H_8 * mds32_r5[30]) ^ (ret3_H_16_L_8 * mds32_r5[14]) ^ (ret3_L_16_H_8 * mds32_r6[30]) ^ (ret3_L_16_L_8 * mds32_r6[14]) ^ (ret4_H_16_H_8 * mds32_r7[30]) ^ (ret4_H_16_L_8 * mds32_r7[14]) ^ (ret4_L_16_H_8 * mds32_r8[30]) ^ (ret4_L_16_L_8 * mds32_r8[14])
        ret1_L_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[29]) ^ (ret1_H_16_L_8 * mds32_r1[13]) ^ (ret1_L_16_H_8 * mds32_r2[29]) ^ (ret1_L_16_L_8 * mds32_r2[13]) ^ (ret2_H_16_H_8 * mds32_r3[29]) ^ (ret2_H_16_L_8 * mds32_r3[13]) ^ (ret2_L_16_H_8 * mds32_r4[29]) ^ (ret2_L_16_L_8 * mds32_r4[13]) ^ (ret3_H_16_H_8 * mds32_r5[29]) ^ (ret3_H_16_L_8 * mds32_r5[13]) ^ (ret3_L_16_H_8 * mds32_r6[29]) ^ (ret3_L_16_L_8 * mds32_r6[13]) ^ (ret4_H_16_H_8 * mds32_r7[29]) ^ (ret4_H_16_L_8 * mds32_r7[13]) ^ (ret4_L_16_H_8 * mds32_r8[29]) ^ (ret4_L_16_L_8 * mds32_r8[13])
        ret1_L_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[28]) ^ (ret1_H_16_L_8 * mds32_r1[12]) ^ (ret1_L_16_H_8 * mds32_r2[28]) ^ (ret1_L_16_L_8 * mds32_r2[12]) ^ (ret2_H_16_H_8 * mds32_r3[28]) ^ (ret2_H_16_L_8 * mds32_r3[12]) ^ (ret2_L_16_H_8 * mds32_r4[28]) ^ (ret2_L_16_L_8 * mds32_r4[12]) ^ (ret3_H_16_H_8 * mds32_r5[28]) ^ (ret3_H_16_L_8 * mds32_r5[12]) ^ (ret3_L_16_H_8 * mds32_r6[28]) ^ (ret3_L_16_L_8 * mds32_r6[12]) ^ (ret4_H_16_H_8 * mds32_r7[28]) ^ (ret4_H_16_L_8 * mds32_r7[12]) ^ (ret4_L_16_H_8 * mds32_r8[28]) ^ (ret4_L_16_L_8 * mds32_r8[12])
        ret2_H_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[27]) ^ (ret1_H_16_L_8 * mds32_r1[11]) ^ (ret1_L_16_H_8 * mds32_r2[27]) ^ (ret1_L_16_L_8 * mds32_r2[11]) ^ (ret2_H_16_H_8 * mds32_r3[27]) ^ (ret2_H_16_L_8 * mds32_r3[11]) ^ (ret2_L_16_H_8 * mds32_r4[27]) ^ (ret2_L_16_L_8 * mds32_r4[11]) ^ (ret3_H_16_H_8 * mds32_r5[27]) ^ (ret3_H_16_L_8 * mds32_r5[11]) ^ (ret3_L_16_H_8 * mds32_r6[27]) ^ (ret3_L_16_L_8 * mds32_r6[11]) ^ (ret4_H_16_H_8 * mds32_r7[27]) ^ (ret4_H_16_L_8 * mds32_r7[11]) ^ (ret4_L_16_H_8 * mds32_r8[27]) ^ (ret4_L_16_L_8 * mds32_r8[11])
        ret2_H_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[26]) ^ (ret1_H_16_L_8 * mds32_r1[10]) ^ (ret1_L_16_H_8 * mds32_r2[26]) ^ (ret1_L_16_L_8 * mds32_r2[10]) ^ (ret2_H_16_H_8 * mds32_r3[26]) ^ (ret2_H_16_L_8 * mds32_r3[10]) ^ (ret2_L_16_H_8 * mds32_r4[26]) ^ (ret2_L_16_L_8 * mds32_r4[10]) ^ (ret3_H_16_H_8 * mds32_r5[26]) ^ (ret3_H_16_L_8 * mds32_r5[10]) ^ (ret3_L_16_H_8 * mds32_r6[26]) ^ (ret3_L_16_L_8 * mds32_r6[10]) ^ (ret4_H_16_H_8 * mds32_r7[26]) ^ (ret4_H_16_L_8 * mds32_r7[10]) ^ (ret4_L_16_H_8 * mds32_r8[26]) ^ (ret4_L_16_L_8 * mds32_r8[10])
        ret2_L_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[25]) ^ (ret1_H_16_L_8 * mds32_r1[9]) ^ (ret1_L_16_H_8 * mds32_r2[25]) ^ (ret1_L_16_L_8 * mds32_r2[9]) ^ (ret2_H_16_H_8 * mds32_r3[25]) ^ (ret2_H_16_L_8 * mds32_r3[9]) ^ (ret2_L_16_H_8 * mds32_r4[25]) ^ (ret2_L_16_L_8 * mds32_r4[9]) ^ (ret3_H_16_H_8 * mds32_r5[25]) ^ (ret3_H_16_L_8 * mds32_r5[9]) ^ (ret3_L_16_H_8 * mds32_r6[25]) ^ (ret3_L_16_L_8 * mds32_r6[9]) ^ (ret4_H_16_H_8 * mds32_r7[25]) ^ (ret4_H_16_L_8 * mds32_r7[9]) ^ (ret4_L_16_H_8 * mds32_r8[25]) ^ (ret4_L_16_L_8 * mds32_r8[9])
        ret2_L_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[24]) ^ (ret1_H_16_L_8 * mds32_r1[8]) ^ (ret1_L_16_H_8 * mds32_r2[24]) ^ (ret1_L_16_L_8 * mds32_r2[8]) ^ (ret2_H_16_H_8 * mds32_r3[24]) ^ (ret2_H_16_L_8 * mds32_r3[8]) ^ (ret2_L_16_H_8 * mds32_r4[24]) ^ (ret2_L_16_L_8 * mds32_r4[8]) ^ (ret3_H_16_H_8 * mds32_r5[24]) ^ (ret3_H_16_L_8 * mds32_r5[8]) ^ (ret3_L_16_H_8 * mds32_r6[24]) ^ (ret3_L_16_L_8 * mds32_r6[8]) ^ (ret4_H_16_H_8 * mds32_r7[24]) ^ (ret4_H_16_L_8 * mds32_r7[8]) ^ (ret4_L_16_H_8 * mds32_r8[24]) ^ (ret4_L_16_L_8 * mds32_r8[8])
        ret3_H_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[23]) ^ (ret1_H_16_L_8 * mds32_r1[7]) ^ (ret1_L_16_H_8 * mds32_r2[23]) ^ (ret1_L_16_L_8 * mds32_r2[7]) ^ (ret2_H_16_H_8 * mds32_r3[23]) ^ (ret2_H_16_L_8 * mds32_r3[7]) ^ (ret2_L_16_H_8 * mds32_r4[23]) ^ (ret2_L_16_L_8 * mds32_r4[7]) ^ (ret3_H_16_H_8 * mds32_r5[23]) ^ (ret3_H_16_L_8 * mds32_r5[7]) ^ (ret3_L_16_H_8 * mds32_r6[23]) ^ (ret3_L_16_L_8 * mds32_r6[7]) ^ (ret4_H_16_H_8 * mds32_r7[23]) ^ (ret4_H_16_L_8 * mds32_r7[7]) ^ (ret4_L_16_H_8 * mds32_r8[23]) ^ (ret4_L_16_L_8 * mds32_r8[7])
        ret3_H_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[22]) ^ (ret1_H_16_L_8 * mds32_r1[6]) ^ (ret1_L_16_H_8 * mds32_r2[22]) ^ (ret1_L_16_L_8 * mds32_r2[6]) ^ (ret2_H_16_H_8 * mds32_r3[22]) ^ (ret2_H_16_L_8 * mds32_r3[6]) ^ (ret2_L_16_H_8 * mds32_r4[22]) ^ (ret2_L_16_L_8 * mds32_r4[6]) ^ (ret3_H_16_H_8 * mds32_r5[22]) ^ (ret3_H_16_L_8 * mds32_r5[6]) ^ (ret3_L_16_H_8 * mds32_r6[22]) ^ (ret3_L_16_L_8 * mds32_r6[6]) ^ (ret4_H_16_H_8 * mds32_r7[22]) ^ (ret4_H_16_L_8 * mds32_r7[6]) ^ (ret4_L_16_H_8 * mds32_r8[22]) ^ (ret4_L_16_L_8 * mds32_r8[6])
        ret3_L_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[21]) ^ (ret1_H_16_L_8 * mds32_r1[5]) ^ (ret1_L_16_H_8 * mds32_r2[21]) ^ (ret1_L_16_L_8 * mds32_r2[5]) ^ (ret2_H_16_H_8 * mds32_r3[21]) ^ (ret2_H_16_L_8 * mds32_r3[5]) ^ (ret2_L_16_H_8 * mds32_r4[21]) ^ (ret2_L_16_L_8 * mds32_r4[5]) ^ (ret3_H_16_H_8 * mds32_r5[21]) ^ (ret3_H_16_L_8 * mds32_r5[5]) ^ (ret3_L_16_H_8 * mds32_r6[21]) ^ (ret3_L_16_L_8 * mds32_r6[5]) ^ (ret4_H_16_H_8 * mds32_r7[21]) ^ (ret4_H_16_L_8 * mds32_r7[5]) ^ (ret4_L_16_H_8 * mds32_r8[21]) ^ (ret4_L_16_L_8 * mds32_r8[5])
        ret3_L_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[20]) ^ (ret1_H_16_L_8 * mds32_r1[4]) ^ (ret1_L_16_H_8 * mds32_r2[20]) ^ (ret1_L_16_L_8 * mds32_r2[4]) ^ (ret2_H_16_H_8 * mds32_r3[20]) ^ (ret2_H_16_L_8 * mds32_r3[4]) ^ (ret2_L_16_H_8 * mds32_r4[20]) ^ (ret2_L_16_L_8 * mds32_r4[4]) ^ (ret3_H_16_H_8 * mds32_r5[20]) ^ (ret3_H_16_L_8 * mds32_r5[4]) ^ (ret3_L_16_H_8 * mds32_r6[20]) ^ (ret3_L_16_L_8 * mds32_r6[4]) ^ (ret4_H_16_H_8 * mds32_r7[20]) ^ (ret4_H_16_L_8 * mds32_r7[4]) ^ (ret4_L_16_H_8 * mds32_r8[20]) ^ (ret4_L_16_L_8 * mds32_r8[4])
        ret4_H_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[19]) ^ (ret1_H_16_L_8 * mds32_r1[3]) ^ (ret1_L_16_H_8 * mds32_r2[19]) ^ (ret1_L_16_L_8 * mds32_r2[3]) ^ (ret2_H_16_H_8 * mds32_r3[19]) ^ (ret2_H_16_L_8 * mds32_r3[3]) ^ (ret2_L_16_H_8 * mds32_r4[19]) ^ (ret2_L_16_L_8 * mds32_r4[3]) ^ (ret3_H_16_H_8 * mds32_r5[19]) ^ (ret3_H_16_L_8 * mds32_r5[3]) ^ (ret3_L_16_H_8 * mds32_r6[19]) ^ (ret3_L_16_L_8 * mds32_r6[3]) ^ (ret4_H_16_H_8 * mds32_r7[19]) ^ (ret4_H_16_L_8 * mds32_r7[3]) ^ (ret4_L_16_H_8 * mds32_r8[19]) ^ (ret4_L_16_L_8 * mds32_r8[3])
        ret4_H_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[18]) ^ (ret1_H_16_L_8 * mds32_r1[2]) ^ (ret1_L_16_H_8 * mds32_r2[18]) ^ (ret1_L_16_L_8 * mds32_r2[2]) ^ (ret2_H_16_H_8 * mds32_r3[18]) ^ (ret2_H_16_L_8 * mds32_r3[2]) ^ (ret2_L_16_H_8 * mds32_r4[18]) ^ (ret2_L_16_L_8 * mds32_r4[2]) ^ (ret3_H_16_H_8 * mds32_r5[18]) ^ (ret3_H_16_L_8 * mds32_r5[2]) ^ (ret3_L_16_H_8 * mds32_r6[18]) ^ (ret3_L_16_L_8 * mds32_r6[2]) ^ (ret4_H_16_H_8 * mds32_r7[18]) ^ (ret4_H_16_L_8 * mds32_r7[2]) ^ (ret4_L_16_H_8 * mds32_r8[18]) ^ (ret4_L_16_L_8 * mds32_r8[2])
        ret4_L_16_H_8 =  (ret1_H_16_H_8 * mds32_r1[17]) ^ (ret1_H_16_L_8 * mds32_r1[1]) ^ (ret1_L_16_H_8 * mds32_r2[17]) ^ (ret1_L_16_L_8 * mds32_r2[1]) ^ (ret2_H_16_H_8 * mds32_r3[17]) ^ (ret2_H_16_L_8 * mds32_r3[1]) ^ (ret2_L_16_H_8 * mds32_r4[17]) ^ (ret2_L_16_L_8 * mds32_r4[1]) ^ (ret3_H_16_H_8 * mds32_r5[17]) ^ (ret3_H_16_L_8 * mds32_r5[1]) ^ (ret3_L_16_H_8 * mds32_r6[17]) ^ (ret3_L_16_L_8 * mds32_r6[1]) ^ (ret4_H_16_H_8 * mds32_r7[17]) ^ (ret4_H_16_L_8 * mds32_r7[1]) ^ (ret4_L_16_H_8 * mds32_r8[17]) ^ (ret4_L_16_L_8 * mds32_r8[1])
        ret4_L_16_L_8 =  (ret1_H_16_H_8 * mds32_r1[16]) ^ (ret1_H_16_L_8 * mds32_r1[0]) ^ (ret1_L_16_H_8 * mds32_r2[16]) ^ (ret1_L_16_L_8 * mds32_r2[0]) ^ (ret2_H_16_H_8 * mds32_r3[16]) ^ (ret2_H_16_L_8 * mds32_r3[0]) ^ (ret2_L_16_H_8 * mds32_r4[16]) ^ (ret2_L_16_L_8 * mds32_r4[0]) ^ (ret3_H_16_H_8 * mds32_r5[16]) ^ (ret3_H_16_L_8 * mds32_r5[0]) ^ (ret3_L_16_H_8 * mds32_r6[16]) ^ (ret3_L_16_L_8 * mds32_r6[0]) ^ (ret4_H_16_H_8 * mds32_r7[16]) ^ (ret4_H_16_L_8 * mds32_r7[0]) ^ (ret4_L_16_H_8 * mds32_r8[16]) ^ (ret4_L_16_L_8 * mds32_r8[0])
        ...
        self._RF[ret] = result = (s1<<96) + (s2<<64) + (s3<<32) + s4
		# Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgfmulx    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1):
            ...
            result = self._RF[ret1 + '_e0'] = (poly * self._RF['dim']) % self._RF['range']
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgfmul8x    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1):
            ...
            poly = (coff[31] * pow(self._RF['dim'], 31) + coff[30] * pow(self._RF['dim'], 30) + coff[29] * pow(self._RF['dim'], 29) + coff[28] * pow(self._RF['dim'], 28) + 
                    ...
                    coff[7] * pow(self._RF['dim'], 7) + coff[6] * pow(self._RF['dim'], 6) + coff[5] * pow(self._RF['dim'], 5) + coff[4] * pow(self._RF['dim'], 4) + 
                    coff[3] * pow(self._RF['dim'], 3) + coff[2] * pow(self._RF['dim'], 2) + coff[1] * pow(self._RF['dim'], 1) + coff[0] * pow(self._RF['dim'], 0))
            result = self._RF[ret1 + '_e0'] = (poly * pow(self._RF['dim'], 8)) % self._RF['range']
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgfmulb    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            self._Log.write("Source Reg 1 : %s = %x  \nSource Reg 2 : %s = %x  \n"%(r1, self._RF[r1 + '_e0'], r2, self._RF[r2 + '_e0']))
            coff = bit32(self._RF[r1])
            poly = (coff[31] * pow(self._RF['dim'], 31) + coff[30] * pow(self._RF['dim'], 30) + coff[29] * pow(self._RF['dim'], 29) + coff[28] * pow(self._RF['dim'], 28) + 
                    coff[27] * pow(self._RF['dim'], 27) + coff[26] * pow(self._RF['dim'], 26) + coff[25] * pow(self._RF['dim'], 25) + coff[24] * pow(self._RF['dim'], 24) + 
                    coff[23] * pow(self._RF['dim'], 23) + coff[22] * pow(self._RF['dim'], 22) + coff[21] * pow(self._RF['dim'], 21) + coff[20] * pow(self._RF['dim'], 20) + 
            ...
            result = self._RF[ret1 + '_e0'] = (poly * byte) % self._RF['range']
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgfmulc    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret1) or self._RF.IsLocked(r1):
            ...
            poly = (coff[31] * pow(self._RF['dim'], 31) + coff[30] * pow(self._RF['dim'], 30) + coff[29] * pow(self._RF['dim'], 29) + coff[28] * pow(self._RF['dim'], 28) + 
                    ...
                    coff[7] * pow(self._RF['dim'], 7) + coff[6] * pow(self._RF['dim'], 6) + coff[5] * pow(self._RF['dim'], 5) + coff[4] * pow(self._RF['dim'], 4) + 
                    coff[3] * pow(self._RF['dim'], 3) + coff[2] * pow(self._RF['dim'], 2) + coff[1] * pow(self._RF['dim'], 1) + coff[0] * pow(self._RF['dim'], 0))
            result = self._RF[ret1] = (poly * self._RF['C']) % self._RF['range']
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_matc    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            rs1 = self._RF[r1] & 0xffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, s1))
            s1, s2, s3, s4 = bit32_to_8(rs1)
            s11, s21, s31, s41 = bit32_to_8(self._RF['mat1'])
            s12, s22, s32, s42 = bit32_to_8(self._RF['mat2'])
            s13, s23, s33, s43 = bit32_to_8(self._RF['mat3'])
            s14, s24, s34, s44 = bit32_to_8(self._RF['mat4'])
            s21 = ex8(s11, s1, self._RF['gen_poly2_8']) ^ ex8(s12, s2, self._RF['gen_poly2_8']) ^ ex8(s13, s3, self._RF['gen_poly2_8']) ^ ex8(s14, s4, self._RF['gen_poly2_8'])
            s22 = ex8(s21, s1, self._RF['gen_poly2_8']) ^ ex8(s22, s2, self._RF['gen_poly2_8']) ^ ex8(s23, s3, self._RF['gen_poly2_8']) ^ ex8(s24, s4, self._RF['gen_poly2_8'])
            s23 = ex8(s31, s1, self._RF['gen_poly2_8']) ^ ex8(s32, s2, self._RF['gen_poly2_8']) ^ ex8(s33, s3, self._RF['gen_poly2_8']) ^ ex8(s34, s4, self._RF['gen_poly2_8'])
            s24 = ex8(s41, s1, self._RF['gen_poly2_8']) ^ ex8(s42, s2, self._RF['gen_poly2_8']) ^ ex8(s43, s3, self._RF['gen_poly2_8']) ^ ex8(s44, s4, self._RF['gen_poly2_8'])
            result = self._RF[ret] = (s21<<24)	+ (s22<<16) + (s23<<8) + s24		
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_mat    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, (self._RF[r1 + '_e4']<<128)+(self._RF[r1+'_e3']<<96)+(self._RF[r1+'_e2']<<64)+(self._RF[r1+'_e1']<<32)+self._RF[r1+'_e0']))
            s1, s2, s3, s4 = bit32_to_8((self._RF[r1]>>128) & 0xffffffff)
            s11, s21, s31, s41 = bit32_to_8((self._RF[r1]>>96) & 0xffffffff)
            s12, s22, s32, s42 = bit32_to_8((self._RF[r1]>>64) & 0xffffffff)
            s13, s23, s33, s43 = bit32_to_8((self._RF[r1]>>32) & 0xffffffff)
            s14, s24, s34, s44 = bit32_to_8(self._RF[r1] & 0xffffffff)
            s21 = ex8(s11, s1, self._RF['gen_poly2_8']) ^ ex8(s12, s2, self._RF['gen_poly2_8']) ^ ex8(s13, s3, self._RF['gen_poly2_8']) ^ ex8(s14, s4, self._RF['gen_poly2_8']) 
            s22 = ex8(s21, s1, self._RF['gen_poly2_8']) ^ ex8(s22, s2, self._RF['gen_poly2_8']) ^ ex8(s23, s3, self._RF['gen_poly2_8']) ^ ex8(s24, s4, self._RF['gen_poly2_8']) 
            s23 = ex8(s31, s1, self._RF['gen_poly2_8']) ^ ex8(s32, s2, self._RF['gen_poly2_8']) ^ ex8(s33, s3, self._RF['gen_poly2_8']) ^ ex8(s34, s4, self._RF['gen_poly2_8']) 
            s24 = ex8(s41, s1, self._RF['gen_poly2_8']) ^ ex8(s42, s2, self._RF['gen_poly2_8']) ^ ex8(s43, s3, self._RF['gen_poly2_8']) ^ ex8(s44, s4, self._RF['gen_poly2_8']) 
    ...
    def _Execqgf8_matcv    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            s1 = self._RF[r1] & 0xffffffffffffffffffffffffffffffff
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, s1))
            c11, c21, c31, c41 = bit32_to_8(self._RF['mat1'])
            c12, c22, c32, c42 = bit32_to_8(self._RF['mat2'])
            c13, c23, c33, c43 = bit32_to_8(self._RF['mat3'])
            c14, c24, c34, c44 = bit32_to_8(self._RF['mat4'])
            s11, s21, s31, s41 = bit32_to_8((self._RF[r1]>>96) & 0xffffffff)
            s12, s22, s32, s42 = bit32_to_8((self._RF[r1]>>64) & 0xffffffff)
            s13, s23, s33, s43 = bit32_to_8((self._RF[r1]>>32) & 0xffffffff)
            s14, s24, s34, s44 = bit32_to_8(self._RF[r1] & 0xffffffff)
            ...
            self._RF[ret] = (s1<<96) + (s2<<64) + (s3<<32) + s4			
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqgf8_matv    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1):
            ...
            self._Log.write("Source Reg 1 : %s = %x  \n"%(r1, self._RF[r1]))
            c11, c21, c31, c41 = bit32_to_8((self._RF[r1]>>224) & 0xffffffff)
            c12, c22, c32, c42 = bit32_to_8((self._RF[r1]>>192) & 0xffffffff)
            c13, c23, c33, c43 = bit32_to_8((self._RF[r1]>>160) & 0xffffffff)
            c14, c24, c34, c44 = bit32_to_8((self._RF[r1]>>128) & 0xffffffff)
            s11, s21, s31, s41 = bit32_to_8((self._RF[r1]>>96) & 0xffffffff)
            s12, s22, s32, s42 = bit32_to_8((self._RF[r1]>>64) & 0xffffffff)
            s13, s23, s33, s43 = bit32_to_8((self._RF[r1]>>32) & 0xffffffff)
            s14, s24, s34, s44 = bit32_to_8(self._RF[r1] & 0xffffffff)
    ...
    def _Execqbfmul32l    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            tempt = 0
            for i in range(32):
                tempt = tempt ^ ((s1 * split[i])<<pow(2, i)) 
            result = self._RF[ret] = tempt & 0xffffffff		
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqbfmul32h    (self, Asm="", PC=None):
        ...
        if self._RF.IsLocked(ret) or self._RF.IsLocked(r1) or self._RF.IsLocked(r2):
            ...
            for i in range(32):
                tempt = tempt ^ ((s1 * split[i])<<pow(2, i)) 
            result = self._RF[ret] = (tempt>>32) & 0xffffffff	
            # Information to be reported
            Matched, Instr = self._InstrSet.MatchedOnce(Asm)
            self.IncreaseIC(Instr)
            self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqslfsr128(self, Asm="", PC=None):
        ...
        for i in range(0, shift):
            ...
            self._RF[ret] = result = ((s0 & ~(1 << 127)) << 1) | bit			
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
        ...
        for i in range(0, shift):
            ...
            for j in range(0,255):
                if ((self._RF["lfsr_xor"] >> j) & 1) == 1:
                    bit = bit ^ ((s0 >> j) & 1)
            bit = bit ^ ((self._RF["rs"] >> i) & 1)
            self._RF[ret] = result  = ((s0 & ~(1 << 255)) << 1) | bit
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
        ...
        for i in range(0, shift):
            ...
            for j in range(0,511):
                if ((self._RF["lfsr_xor"] >> j) & 1) == 1:
                    bit = (bit ^ (s >> j) & 1)
            bit = bit ^ ((self._RF["rs"] >> i) & 1)
            tempt0 = (((s & ~(1 << 511)) << 1) | bit)>>256
            tempt1 = (((s & ~(1 << 511)) << 1) | bit) & (pow(2, 256) - 1)
            self._RF[ret0] = tempt0
            self._RF[ret1] = tempt1
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
        self.IncreaseCycles(Instr, "Parallel", self._InstrSet.GetCycles(Instr))
        self._Log.write("Result0 : %s = 0x%x \n Result1 : %s = %x \n" % (ret0, tempt0, ret1, tempt1))
        return True, PC + 1
    ...
    def _Execqdlfsr128(self, Asm="", PC=None):
        ...
        for i in range(0, shift):
            if ((s0 >> 127) & 1) == 1:
                tmp = ((s0 ^ self._RF["lfsr_xor"]) & ~(1 << 127)) << 1
                self._RF[ret] = tmp | (((self._RF["rs"] >> i) & 1) ^ 1)
            else:
                self._RF[ret] = ((s0 & ~(1 << 127)) << 1) | ((self._RF["rs"] >> i) & 1)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
    ...
    def _Execqdlfsr256(self, Asm="", PC=None):
        ...
        for i in range(0, shift):
            if ((s0 >> 255) & 1) == 1:
                tmp = ((s0 ^ self._RF["lfsr_xor"]) & ~(1 << 255)) << 1
                self._RF[ret] = tmp | (((self._RF["rs"] >> i) & 1) ^ 1)
            else:
                self._RF[ret] = ((s0 & ~(1 << 255)) << 1) | ((self._RF["rs"] >> i) & 1)
        # Information to be reported
        Matched, Instr = self._InstrSet.MatchedOnce(Asm)
        self.IncreaseIC(Instr)
        self.IncreaseCycles(Instr, "Serial", self._InstrSet.GetCycles(Instr))
...
class CryptoRegFiles:
    def __init__(self):
        ...
        CSR = {"CY" : {"Value": 0, "Locked": False},
               ...
               "mat1" : {"Value": 0x02010103, "Locked": False},
               "mat2" : {"Value": 0x03020101, "Locked": False},
               "mat3" : {"Value": 0x01030201, "Locked": False},
               "mat4" : {"Value": 0x01010302, "Locked": False},
        ...
        LUT8X8 = {
            "LUT8X8_0": {"Value": 0, "Locked": False},
            "LUT8X8_1": {"Value": 1, "Locked": False},
            "LUT8X8_2": {"Value": 2, "Locked": False},
            "LUT8X8_3": {"Value": 3, "Locked": False},
            "LUT8X8_4": {"Value": 4, "Locked": False},
            "LUT8X8_5": {"Value": 5, "Locked": False},
            "LUT8X8_6": {"Value": 6, "Locked": False},
            "LUT8X8_7": {"Value": 7, "Locked": False},
            "LUT8X8_8": {"Value": 8, "Locked": False},
            "LUT8X8_9": {"Value": 9, "Locked": False},
            "LUT8X8_10": {"Value": 10, "Locked": False},
            "LUT8X8_11": {"Value": 11, "Locked": False},
            "LUT8X8_12": {"Value": 12, "Locked": False},
            "LUT8X8_13": {"Value": 13, "Locked": False},
            "LUT8X8_14": {"Value": 14, "Locked": False},
            "LUT8X8_15": {"Value": 15, "Locked": False},
            "LUT8X8_16": {"Value": 16, "Locked": False},
            "LUT8X8_17": {"Value": 17, "Locked": False},
            "LUT8X8_18": {"Value": 18, "Locked": False},
            "LUT8X8_19": {"Value": 19, "Locked": False},
            "LUT8X8_20": {"Value": 20, "Locked": False},
            "LUT8X8_21": {"Value": 21, "Locked": False},
            "LUT8X8_22": {"Value": 22, "Locked": False},
            "LUT8X8_23": {"Value": 23, "Locked": False},
            "LUT8X8_24": {"Value": 24, "Locked": False},
            "LUT8X8_25": {"Value": 25, "Locked": False},
            "LUT8X8_26": {"Value": 26, "Locked": False},
            "LUT8X8_27": {"Value": 27, "Locked": False},
            "LUT8X8_28": {"Value": 28, "Locked": False},
            "LUT8X8_29": {"Value": 29, "Locked": False},
            "LUT8X8_30": {"Value": 30, "Locked": False},
            "LUT8X8_31": {"Value": 31, "Locked": False},
            "LUT8X8_32": {"Value": 32, "Locked": False},
            "LUT8X8_33": {"Value": 33, "Locked": False},
            "LUT8X8_34": {"Value": 34, "Locked": False},
            "LUT8X8_35": {"Value": 35, "Locked": False},
            "LUT8X8_36": {"Value": 36, "Locked": False},
            "LUT8X8_37": {"Value": 37, "Locked": False},
            "LUT8X8_38": {"Value": 38, "Locked": False},
            "LUT8X8_39": {"Value": 39, "Locked": False},
            "LUT8X8_40": {"Value": 40, "Locked": False},
            "LUT8X8_41": {"Value": 41, "Locked": False},
            "LUT8X8_42": {"Value": 42, "Locked": False},
            "LUT8X8_43": {"Value": 43, "Locked": False},
            "LUT8X8_44": {"Value": 44, "Locked": False},
            "LUT8X8_45": {"Value": 45, "Locked": False},
            "LUT8X8_46": {"Value": 46, "Locked": False},
            "LUT8X8_47": {"Value": 47, "Locked": False},
            "LUT8X8_48": {"Value": 48, "Locked": False},
            "LUT8X8_49": {"Value": 49, "Locked": False},
            "LUT8X8_50": {"Value": 50, "Locked": False},
            "LUT8X8_51": {"Value": 51, "Locked": False},
            "LUT8X8_52": {"Value": 52, "Locked": False},
            "LUT8X8_53": {"Value": 53, "Locked": False},
            "LUT8X8_54": {"Value": 54, "Locked": False},
            "LUT8X8_55": {"Value": 55, "Locked": False},
            "LUT8X8_56": {"Value": 56, "Locked": False},
            "LUT8X8_57": {"Value": 57, "Locked": False},
            "LUT8X8_58": {"Value": 58, "Locked": False},
            "LUT8X8_59": {"Value": 59, "Locked": False},
            "LUT8X8_60": {"Value": 60, "Locked": False},
            "LUT8X8_61": {"Value": 61, "Locked": False},
            "LUT8X8_62": {"Value": 62, "Locked": False},
            "LUT8X8_63": {"Value": 63, "Locked": False},
            "LUT8X8_64": {"Value": 64, "Locked": False},
            "LUT8X8_65": {"Value": 65, "Locked": False},
            "LUT8X8_66": {"Value": 66, "Locked": False},
            "LUT8X8_67": {"Value": 67, "Locked": False},
            "LUT8X8_68": {"Value": 68, "Locked": False},
            "LUT8X8_69": {"Value": 69, "Locked": False},
            "LUT8X8_70": {"Value": 70, "Locked": False},
            "LUT8X8_71": {"Value": 71, "Locked": False},
            "LUT8X8_72": {"Value": 72, "Locked": False},
            "LUT8X8_73": {"Value": 73, "Locked": False},
            "LUT8X8_74": {"Value": 74, "Locked": False},
            "LUT8X8_75": {"Value": 75, "Locked": False},
            "LUT8X8_76": {"Value": 76, "Locked": False},
            "LUT8X8_77": {"Value": 77, "Locked": False},
            "LUT8X8_78": {"Value": 78, "Locked": False},
            "LUT8X8_79": {"Value": 79, "Locked": False},
            "LUT8X8_80": {"Value": 80, "Locked": False},
            "LUT8X8_81": {"Value": 81, "Locked": False},
            "LUT8X8_82": {"Value": 82, "Locked": False},
            "LUT8X8_83": {"Value": 83, "Locked": False},
            "LUT8X8_84": {"Value": 84, "Locked": False},
            "LUT8X8_85": {"Value": 85, "Locked": False},
            "LUT8X8_86": {"Value": 86, "Locked": False},
            "LUT8X8_87": {"Value": 87, "Locked": False},
            "LUT8X8_88": {"Value": 88, "Locked": False},
            "LUT8X8_89": {"Value": 89, "Locked": False},
            "LUT8X8_90": {"Value": 90, "Locked": False},
            "LUT8X8_91": {"Value": 91, "Locked": False},
            "LUT8X8_92": {"Value": 92, "Locked": False},
            "LUT8X8_93": {"Value": 93, "Locked": False},
            "LUT8X8_94": {"Value": 94, "Locked": False},
            "LUT8X8_95": {"Value": 95, "Locked": False},
            "LUT8X8_96": {"Value": 96, "Locked": False},
            "LUT8X8_97": {"Value": 97, "Locked": False},
            "LUT8X8_98": {"Value": 98, "Locked": False},
            "LUT8X8_99": {"Value": 99, "Locked": False},
            "LUT8X8_100": {"Value": 100, "Locked": False},
            "LUT8X8_101": {"Value": 101, "Locked": False},
            "LUT8X8_102": {"Value": 102, "Locked": False},
            "LUT8X8_103": {"Value": 103, "Locked": False},
            "LUT8X8_104": {"Value": 104, "Locked": False},
            "LUT8X8_105": {"Value": 105, "Locked": False},
            "LUT8X8_106": {"Value": 106, "Locked": False},
            "LUT8X8_107": {"Value": 107, "Locked": False},
            "LUT8X8_108": {"Value": 108, "Locked": False},
            "LUT8X8_109": {"Value": 109, "Locked": False},
            "LUT8X8_110": {"Value": 110, "Locked": False},
            "LUT8X8_111": {"Value": 111, "Locked": False},
            "LUT8X8_112": {"Value": 112, "Locked": False},
            "LUT8X8_113": {"Value": 113, "Locked": False},
            "LUT8X8_114": {"Value": 114, "Locked": False},
            "LUT8X8_115": {"Value": 115, "Locked": False},
            "LUT8X8_116": {"Value": 116, "Locked": False},
            "LUT8X8_117": {"Value": 117, "Locked": False},
            "LUT8X8_118": {"Value": 118, "Locked": False},
            "LUT8X8_119": {"Value": 119, "Locked": False},
            "LUT8X8_120": {"Value": 120, "Locked": False},
            "LUT8X8_121": {"Value": 121, "Locked": False},
            "LUT8X8_122": {"Value": 122, "Locked": False},
            "LUT8X8_123": {"Value": 123, "Locked": False},
            "LUT8X8_124": {"Value": 124, "Locked": False},
            "LUT8X8_125": {"Value": 125, "Locked": False},
            "LUT8X8_126": {"Value": 126, "Locked": False},
            "LUT8X8_127": {"Value": 127, "Locked": False},
            "LUT8X8_128": {"Value": 128, "Locked": False},
            "LUT8X8_129": {"Value": 129, "Locked": False},
            "LUT8X8_130": {"Value": 130, "Locked": False},
            "LUT8X8_131": {"Value": 131, "Locked": False},
            "LUT8X8_132": {"Value": 132, "Locked": False},
            "LUT8X8_133": {"Value": 133, "Locked": False},
            "LUT8X8_134": {"Value": 134, "Locked": False},
            "LUT8X8_135": {"Value": 135, "Locked": False},
            "LUT8X8_136": {"Value": 136, "Locked": False},
            "LUT8X8_137": {"Value": 137, "Locked": False},
            "LUT8X8_138": {"Value": 138, "Locked": False},
            "LUT8X8_139": {"Value": 139, "Locked": False},
            "LUT8X8_140": {"Value": 140, "Locked": False},
            "LUT8X8_141": {"Value": 141, "Locked": False},
            "LUT8X8_142": {"Value": 142, "Locked": False},
            "LUT8X8_143": {"Value": 143, "Locked": False},
            "LUT8X8_144": {"Value": 144, "Locked": False},
            "LUT8X8_145": {"Value": 145, "Locked": False},
            "LUT8X8_146": {"Value": 146, "Locked": False},
            "LUT8X8_147": {"Value": 147, "Locked": False},
            "LUT8X8_148": {"Value": 148, "Locked": False},
            "LUT8X8_149": {"Value": 149, "Locked": False},
            "LUT8X8_150": {"Value": 150, "Locked": False},
            "LUT8X8_151": {"Value": 151, "Locked": False},
            "LUT8X8_152": {"Value": 152, "Locked": False},
            "LUT8X8_153": {"Value": 153, "Locked": False},
            "LUT8X8_154": {"Value": 154, "Locked": False},
            "LUT8X8_155": {"Value": 155, "Locked": False},
            "LUT8X8_156": {"Value": 156, "Locked": False},
            "LUT8X8_157": {"Value": 157, "Locked": False},
            "LUT8X8_158": {"Value": 158, "Locked": False},
            "LUT8X8_159": {"Value": 159, "Locked": False},
            "LUT8X8_160": {"Value": 160, "Locked": False},
            "LUT8X8_161": {"Value": 161, "Locked": False},
            "LUT8X8_162": {"Value": 162, "Locked": False},
            "LUT8X8_163": {"Value": 163, "Locked": False},
            "LUT8X8_164": {"Value": 164, "Locked": False},
            "LUT8X8_165": {"Value": 165, "Locked": False},
            "LUT8X8_166": {"Value": 166, "Locked": False},
            "LUT8X8_167": {"Value": 167, "Locked": False},
            "LUT8X8_168": {"Value": 168, "Locked": False},
            "LUT8X8_169": {"Value": 169, "Locked": False},
            "LUT8X8_170": {"Value": 170, "Locked": False},
            "LUT8X8_171": {"Value": 171, "Locked": False},
            "LUT8X8_172": {"Value": 172, "Locked": False},
            "LUT8X8_173": {"Value": 173, "Locked": False},
            "LUT8X8_174": {"Value": 174, "Locked": False},
            "LUT8X8_175": {"Value": 175, "Locked": False},
            "LUT8X8_176": {"Value": 176, "Locked": False},
            "LUT8X8_177": {"Value": 177, "Locked": False},
            "LUT8X8_178": {"Value": 178, "Locked": False},
            "LUT8X8_179": {"Value": 179, "Locked": False},
            "LUT8X8_180": {"Value": 180, "Locked": False},
            "LUT8X8_181": {"Value": 181, "Locked": False},
            "LUT8X8_182": {"Value": 182, "Locked": False},
            "LUT8X8_183": {"Value": 183, "Locked": False},
            "LUT8X8_184": {"Value": 184, "Locked": False},
            "LUT8X8_185": {"Value": 185, "Locked": False},
            "LUT8X8_186": {"Value": 186, "Locked": False},
            "LUT8X8_187": {"Value": 187, "Locked": False},
            "LUT8X8_188": {"Value": 188, "Locked": False},
            "LUT8X8_189": {"Value": 189, "Locked": False},
            "LUT8X8_190": {"Value": 190, "Locked": False},
            "LUT8X8_191": {"Value": 191, "Locked": False},
            "LUT8X8_192": {"Value": 192, "Locked": False},
            "LUT8X8_193": {"Value": 193, "Locked": False},
            "LUT8X8_194": {"Value": 194, "Locked": False},
            "LUT8X8_195": {"Value": 195, "Locked": False},
            "LUT8X8_196": {"Value": 196, "Locked": False},
            "LUT8X8_197": {"Value": 197, "Locked": False},
            "LUT8X8_198": {"Value": 198, "Locked": False},
            "LUT8X8_199": {"Value": 199, "Locked": False},
            "LUT8X8_200": {"Value": 200, "Locked": False},
            "LUT8X8_201": {"Value": 201, "Locked": False},
            "LUT8X8_202": {"Value": 202, "Locked": False},
            "LUT8X8_203": {"Value": 203, "Locked": False},
            "LUT8X8_204": {"Value": 204, "Locked": False},
            "LUT8X8_205": {"Value": 205, "Locked": False},
            "LUT8X8_206": {"Value": 206, "Locked": False},
            "LUT8X8_207": {"Value": 207, "Locked": False},
            "LUT8X8_208": {"Value": 208, "Locked": False},
            "LUT8X8_209": {"Value": 209, "Locked": False},
            "LUT8X8_210": {"Value": 210, "Locked": False},
            "LUT8X8_211": {"Value": 211, "Locked": False},
            "LUT8X8_212": {"Value": 212, "Locked": False},
            "LUT8X8_213": {"Value": 213, "Locked": False},
            "LUT8X8_214": {"Value": 214, "Locked": False},
            "LUT8X8_215": {"Value": 215, "Locked": False},
            "LUT8X8_216": {"Value": 216, "Locked": False},
            "LUT8X8_217": {"Value": 217, "Locked": False},
            "LUT8X8_218": {"Value": 218, "Locked": False},
            "LUT8X8_219": {"Value": 219, "Locked": False},
            "LUT8X8_220": {"Value": 220, "Locked": False},
            "LUT8X8_221": {"Value": 221, "Locked": False},
            "LUT8X8_222": {"Value": 222, "Locked": False},
            "LUT8X8_223": {"Value": 223, "Locked": False},
            "LUT8X8_224": {"Value": 224, "Locked": False},
            "LUT8X8_225": {"Value": 225, "Locked": False},
            "LUT8X8_226": {"Value": 226, "Locked": False},
            "LUT8X8_227": {"Value": 227, "Locked": False},
            "LUT8X8_228": {"Value": 228, "Locked": False},
            "LUT8X8_229": {"Value": 229, "Locked": False},
            "LUT8X8_230": {"Value": 230, "Locked": False},
            "LUT8X8_231": {"Value": 231, "Locked": False},
            "LUT8X8_232": {"Value": 232, "Locked": False},
            "LUT8X8_233": {"Value": 233, "Locked": False},
            "LUT8X8_234": {"Value": 234, "Locked": False},
            "LUT8X8_235": {"Value": 235, "Locked": False},
            "LUT8X8_236": {"Value": 236, "Locked": False},
            "LUT8X8_237": {"Value": 237, "Locked": False},
            "LUT8X8_238": {"Value": 238, "Locked": False},
            "LUT8X8_239": {"Value": 239, "Locked": False},
            "LUT8X8_240": {"Value": 240, "Locked": False},
            "LUT8X8_241": {"Value": 241, "Locked": False},
            "LUT8X8_242": {"Value": 242, "Locked": False},
            "LUT8X8_243": {"Value": 243, "Locked": False},
            "LUT8X8_244": {"Value": 244, "Locked": False},
            "LUT8X8_245": {"Value": 245, "Locked": False},
            "LUT8X8_246": {"Value": 246, "Locked": False},
            "LUT8X8_247": {"Value": 247, "Locked": False},
            "LUT8X8_248": {"Value": 248, "Locked": False},
            "LUT8X8_249": {"Value": 249, "Locked": False},
            "LUT8X8_250": {"Value": 250, "Locked": False},
            "LUT8X8_251": {"Value": 251, "Locked": False},
            "LUT8X8_252": {"Value": 252, "Locked": False},
            "LUT8X8_253": {"Value": 253, "Locked": False},
            "LUT8X8_254": {"Value": 254, "Locked": False},
            "LUT8X8_255": {"Value": 255, "Locked": False},
        }
        LUT4X4 = {
...
class CryptoInstrSet:
    def __init__(self):
        ...
        self._InstrParams = {
                ...
                "sd32": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+sd32\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "sd64": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+sd64\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "sd128": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+sd128\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "sd256": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+sd256\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "ld32": {"Cycles": 5,
                   "Exec": None,
                ...
                "ld128": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+ld128\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "ld256": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+ld256\s+(\w+)\,\s+(.+)"),
                   "Params": 0},
                "flkt8m8": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+flkt8m8\s+(.+)"),
                   "Params": 0},
                "flkt4m4": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+flkt4m4\s+(.+)"),
                   "Params": 0},
                "flkt6m4": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+flkt6m4\s+(.+)"),
                   "Params": 0},
                "flkt8m4": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+flkt8m4\s+(.+)"),
                   "Params": 0},
                "flkt8m32": {"Cycles": 5,
                   "Exec": None,
                   "Pattern": re.compile("\s+flkt8m32\s+(.+)"),
                   "Params": 0},
                "li": {"Cycles": 1,
                   "Exec": None,
